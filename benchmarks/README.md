...
Harness output is written under `results/`, which is created on demand
(see `local_load_runner.py`). The cross-phase metrics journal kept during
the 35-day build was maintained alongside the runs, not regenerated by them.
